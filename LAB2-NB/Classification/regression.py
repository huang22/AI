...
def one_hot(filename,voca,length,testType):
    ...
    # 做one hot矩阵   
def not_exit(filename,voca,length,testType):
    if(testType == "classification"):
        mat,classLabel = mat_label(filename)
    else:
        mat,classLabel = mat_label_reg(filename)
      
    oneMat = ones((len(classLabel),len(voca)))
...
def stop_word_class():
    ...
    stop_word = [] 
    index = 0
    for i in range(len(trainOneMat[0])):
        # 相关度在每个情感都是比较大的，则为停用词
        if((pNum[0][i] / pDenom[0] > 0.00001 and pNum[1][i] / pDenom[1] > 0.00001 and pNum[2][i] / pDenom[2] > 0.00001)):
            if((pNum[3][i] / pDenom[3] > 0.00001 and pNum[4][i] /pDenom[4] > 0.00001 and pNum[5][i] / pDenom[5] > 0.00001)):
                stop_word.append(voca[i])
                index += 1
    print stop_word
    print index
        ...
        for k in trainOneMat:
            ...
            for j in range(len(i)):
                if(i[j] != 0):
                    for l in range(6):
                        # 词数 + 1 / 总词数 + V
                        pEmotion[l] = float(pEmotion[l]) * (float(k[j]+1)) /  (float(k.sum() + tmp))
            pFinalEmo += pEmotion
        # 归一化
        pFinalEmo = pFinalEmo / pFinalEmo.sum(axis=0)
        myLabel[index] = (pFinalEmo.T)[0]
...
def test_class(matType = "lalala"):
    
    a,length = vocabulary()
    voca = stop_word_class()
    if matType == "TF_IDF":
    ...
    if matType == "lalala":
           trainMat,trainLabel,trainOneMat = not_exit('train.txt',voca,length,"classification")
           testMat,testLabel,testOneMat = not_exit('test.txt',voca,length,"classification")
    myLabel = []
    ...
    for i in testOneMat:
        ...
        for j in range(len(i)):
            if(i[j] != 1):
                for k in range(6):
                        # 出现该词次数 + 1 / 总词数 + V
                    #pFinalEmo[k] = float(pFinalEmo[k]) * (float(pNum[k][j] + 1) / float(pDenom[k] + tmp))
                    pFinalEmo[k] = float(pFinalEmo[k]) * (float(pNum[k][j] ) / float(pDenom[k] ))
